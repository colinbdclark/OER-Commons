...
GENERAL_SUBJECT_SLUGS = set(GeneralSubject.objects.all().values_list("slug", flat=True))
GRADE_LEVEL_SLUGS = set(GradeLevel.objects.all().values_list("slug", flat=True))
COURSE_MATERIAL_TYPE_SLUGS = set(CourseMaterialType.objects.all().values_list("slug", flat=True))
LIBRARY_MATERIAL_TYPE_SLUGS = set(LibraryMaterialType.objects.all().values_list("slug", flat=True))
MEDIA_FORMAT_SLUGS = set(MediaFormat.objects.all().values_list("slug", flat=True))
LANGUAGE_SLUGS = set(Language.objects.all().values_list("slug", flat=True))
GEO_RELEVANCE_SLUGS = set(GeographicRelevance.objects.all().values_list("slug", flat=True))
...
COURSE_FIELDS = {
  ...
  'CR_COURSE_ID':(
      extract_string, None),
  'CR_TITLE':(
      extract_string, check_required),
  'CR_FCOLM':(
      extract_string_slugify, check_in_list(set(dict(COURSE_OR_MODULE).keys()))),
  'CR_CREATE_DATE':(
      extract_date, None),
  'CR_ENTRY_DATE': (
      lambda x: None, None),
  ...
  'CR_AUTHOR_EMAIL':(
      extract_list, None),
  'CR_AUTHOR_COUNTRY':(
      extract_list_slugify, check_in_list(COUNTRY_SLUGS)),
  'CR_INSTITUTION':(
      extract_string, check_required),
  'CR_URL':(
      extract_string, [check_required, check_URL]),
  'CR_IS_PART_OF_OCW':(
      extract_boolean, None),
  'CR_COLLECTION':(
      extract_string, check_required),
  'CR_SUBJECT':(
      extract_list_slugify, [check_required, check_in_list(GENERAL_SUBJECT_SLUGS)]),
  'CR_MATERIAL_TYPE':(
      extract_list_slugify, [check_required, check_in_list(COURSE_MATERIAL_TYPE_SLUGS)]),
  'CR_MEDIA_FORMATS':(
      extract_list_slugify, [check_required, check_in_list(MEDIA_FORMAT_SLUGS)]),
  'CR_NOTABLE_REQS':(
      extract_string, None),
  'CR_LEVEL':(
      extract_list_slugify, [check_required, check_in_list(GRADE_LEVEL_SLUGS)]),
  'CR_ABSTRACT':(
      extract_string, check_required),
  'CR_KEYWORDS':(
      extract_keywords, check_required),
  'CR_LANGUAGE':(
      extract_list, check_in_list(LANGUAGE_SLUGS)),
  'CR_IRR':(
      extract_list_slugify, check_in_list(GEO_RELEVANCE_SLUGS)),
  'CR_PREREQ_TITLE1':(
      extract_string, None),
  'CR_PREREQ_URL1':(
      extract_string, check_URL),
  'CR_PREREQ_TITLE2':(
      extract_string, None),
  'CR_PREREQ_URL2':(
      extract_string, check_URL),
...
LIBRARY_FIELDS = {
  ...
  'LIB_SUBJECT':(
      extract_list_slugify, [check_required, check_in_list(GENERAL_SUBJECT_SLUGS)]),
  'LIB_MATERIAL_TYPE':(
      extract_list_slugify, [check_required, check_in_list(LIBRARY_MATERIAL_TYPE_SLUGS)]),
  'LIB_MEDIA_FORMATS':(
      extract_list_slugify, [check_required, check_in_list(MEDIA_FORMAT_SLUGS)]),
  'LIB_IS_HOME_PAGE':(
      extract_boolean, None),
    ...
    def clean_file(self):
        ...
        if file.content_type == "application/zip":
            try:
                ...
                raise forms.ValidationError(u"Can't open the archive. Make sure it is a valid ZIP file.")
        ...
        for line_index, row in enumerate(reader):
            ...
            for field_index, raw_value in enumerate(row):
                ...
                try:
                    ...
                    self.add_validation_error(u"Can't extract value: %s" % raw_value,
