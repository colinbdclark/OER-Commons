...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        ...
        # Remove existing duplicated GenericMaterials
        print "Removing duplicated generic materials..."
        prev = None
        for material in orm.GenericMaterial.objects.all().order_by("url", "created_on"):
            url = material.url
            if url == prev:
                material.delete()
                print "Duplicated %s removed..." % url
            prev = url

        print "Done!"

        # Adding unique constraint on 'GenericMaterial', fields ['url']
        db.create_unique('materials_genericmaterial', ['url'])


    def backwards(self, orm):

        # Removing unique constraint on 'GenericMaterial', fields ['url']
        db.delete_unique('materials_genericmaterial', ['url'])
