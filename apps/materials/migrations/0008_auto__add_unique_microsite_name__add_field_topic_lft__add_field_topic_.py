...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        ...
        # Adding unique constraint on 'Microsite', fields ['name']
        db.create_unique('materials_microsite', ['name'])

        # Adding field 'Topic.lft'
        db.add_column('materials_topic', 'lft', self.gf('django.db.models.fields.PositiveIntegerField')(default=0, db_index=True), keep_default=False)

        # Adding field 'Topic.rght'
        db.add_column('materials_topic', 'rght', self.gf('django.db.models.fields.PositiveIntegerField')(default=0, db_index=True), keep_default=False)

        # Adding field 'Topic.tree_id'
        db.add_column('materials_topic', 'tree_id', self.gf('django.db.models.fields.PositiveIntegerField')(default=0, db_index=True), keep_default=False)

        # Adding field 'Topic.level'
        db.add_column('materials_topic', 'level', self.gf('django.db.models.fields.PositiveIntegerField')(default=0, db_index=True), keep_default=False)

        # Adding unique constraint on 'Topic', fields ['microsite', 'name']
        db.create_unique('materials_topic', ['microsite_id', 'name'])


    def backwards(self, orm):
        
        # Removing unique constraint on 'Topic', fields ['microsite', 'name']
        db.delete_unique('materials_topic', ['microsite_id', 'name'])

        # Removing unique constraint on 'Microsite', fields ['name']
        db.delete_unique('materials_microsite', ['name'])

        # Deleting field 'Topic.lft'
        db.delete_column('materials_topic', 'lft')

        # Deleting field 'Topic.rght'
        db.delete_column('materials_topic', 'rght')

        # Deleting field 'Topic.tree_id'
        db.delete_column('materials_topic', 'tree_id')

        # Deleting field 'Topic.level'
        db.delete_column('materials_topic', 'level')
    ...
    models = {
        ...
        'materials.topic': {
            'Meta': {'ordering': "['microsite', 'parent__id', 'id']", 'unique_together': "(('name', 'microsite'),)", 'object_name': 'Topic'},
            'id': ('django.db.models.fields.AutoField', [], {'primary_key': 'True'}),
            'keywords': ('materials.models.common.AutoCreateManyToManyField', [], {'to': "orm['materials.Keyword']", 'symmetrical': 'False'}),
